...
import { Loader2, MessageCircle, Heart, Star } from 'lucide-react';
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {isLoading ? (
        ...
        <div className="glass-card p-12 text-center rounded-3xl">
          ...
          <Link href="/discover" className="inline-flex h-10 items-center justify-center rounded-full bg-primary px-6 text-sm font-medium text-white transition-colors hover:bg-primary/90">
            Go to Discover
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {matches.map((match) => (
            <div key={match.id} className="glass-card p-4 rounded-2xl flex flex-col">
              <div className="flex items-center gap-4 mb-4">
                <div className="relative">
                  <div className="w-16 h-16 rounded-full bg-white/10 overflow-hidden border-2 border-primary">
