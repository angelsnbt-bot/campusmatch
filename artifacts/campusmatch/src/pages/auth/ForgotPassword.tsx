...
export default function ForgotPassword() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    sendOtpMutation.mutate({ data: { email } }, {
      onSuccess: () => { setStep('otp'); setCountdown(60); toast({ title: 'OTP Sent', description: 'Check your email for the 6-digit verification code.' }); },
      onError: (err) => { toast({ title: 'Request failed', description: (err?.data as any)?.error || err?.message || 'Failed to send OTP.', variant: 'destructive' }); },
    });
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) { toast({ title: 'Password too short', description: 'Minimum 8 characters required.', variant: 'destructive' }); return; }
    resetPasswordMutation.mutate({ data: { resetToken, password } }, {
      onSuccess: () => { setStep('done'); toast({ title: 'Password reset successful!', description: 'Redirecting to login...' }); setTimeout(() => setLocation('/login'), 2000); },
      onError: (err) => { toast({ title: 'Reset failed', description: (err?.data as any)?.error || err?.message || 'Failed to reset password.', variant: 'destructive' }); },
    });
  };
