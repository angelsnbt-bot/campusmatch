    ...
    registerMutation.mutate({
      ...
      onError: (err) => {
        ...
        toast({
          ...
          description: errorMessage || 'Could not create account. Please check your connection and try again.',
          variant: 'destructive',
        });
      },
    });
      ...
      <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }}
        ...
        <div className="glass-card p-8 rounded-2xl overflow-hidden">
          ...
          {step === 1 && (
            <>
              {isGoogleAvailable ? (
                <div id="google-signin-btn" className="flex justify-center mb-4 [&>div]:w-full [&>div]:!bg-white/[0.06] [&>div]:!rounded-xl [&>div]:!h-11 [&>div]:!border [&>div]:!border-white/10 [&>div]:!shadow-none [&_iframe]:!h-11" />
              ) : (
                <button disabled className="w-full flex items-center justify-center gap-3 h-11 rounded-xl bg-white/[0.06] border border-white/10 text-white/40 font-medium text-sm cursor-not-allowed mb-4">
