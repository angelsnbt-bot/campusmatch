...
export default function Login() {
  ...
  const { isPending: isGooglePending, isAvailable: isGoogleAvailable } = useGoogleSignIn();
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f0515 0%, #150a20 40%, #1a0f2e 100%)' }}>
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-4">
          <Link href="/">
            <button className="flex items-center gap-2 text-white/50 hover:text-white/80 transition-colors text-sm font-medium group">
        ...
        <div className="glass-card p-8 rounded-2xl overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-white/50 text-sm text-center">
              {mode === 'admin' ? 'Sign in with admin credentials' : 'Sign in to your campus network'}
            </motion.p>
          </div>

          {/* Google Sign In - only on password mode */}
          {mode === 'password' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="mb-4">
              {isGoogleAvailable ? (
                <div id="google-signin-btn" className="flex justify-center [&>div]:w-full [&>div]:!bg-white/[0.06] [&>div]:!rounded-xl [&>div]:!h-12 [&>div]:!border [&>div]:!border-white/10 [&>div]:!shadow-none [&_iframe]:!h-12" />
              ) : (
                <button disabled className="w-full flex items-center justify-center gap-3 h-12 rounded-xl bg-white/[0.06] border border-white/10 text-white/40 font-medium text-sm cursor-not-allowed">
                  <svg className="w-5 h-5 opacity-40" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
          ...
          <div className="flex gap-1 p-1 rounded-xl bg-white/[0.04] mb-6">
            <button onClick={() => setMode('password')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'password' ? 'bg-white/[0.08] text-white' : 'text-white/40 hover:text-white/60'}`}>
              Password
            </button>
            <button onClick={() => setMode('admin')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'admin' ? 'bg-gradient-to-r from-pink-500/20 to-purple-500/20 text-pink-300 border border-pink-500/20' : 'text-white/40 hover:text-white/60'}`}>
              Admin Login
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 }} className="space-y-2">
              ...
              <div className="relative">
                <Input id="email" type="email" placeholder={mode === 'admin' ? 'admin@campusmatch.in' : 'name.branch@college.edu'} value={email} onChange={(e) => setEmail(e.target.value)} required className="input-premium h-12 pl-10 focus-visible:ring-pink-500/50" />
              ...
              <Button type="submit" className="w-full bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white border-0 h-12 text-sm font-semibold shadow-lg shadow-pink-500/20 rounded-xl transition-all hover:shadow-pink-500/30 hover:scale-[1.02] active:scale-[0.98]" disabled={loginMutation.isPending}>
                {loginMutation.isPending ? (
                  ...
                  <span className="flex items-center gap-2">
                    {mode === 'admin' ? 'Sign In as Admin' : 'Sign In'} <ArrowRight className="w-4 h-4" />
                  </span>
                )}
              </Button>
            </motion.div>
