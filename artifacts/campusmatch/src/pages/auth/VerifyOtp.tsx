...
export default function VerifyOtp() {
  ...
  const handleInputChange = (index: number, val: string) => {
    ...
    const newOtp = [...otp];
    newOtp[index] = cleaned[cleaned.length - 1]; // Use last character entered
    setOtp(newOtp);
    ...
    if (completeOtp.length === 6 && index === 5) {
      triggerVerification(completeOtp);
    }
  };
  ...
  const handleKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace') {
      ...
      if (otp[index]) {
        ...
        // If empty, clear previous box and focus it
        newOtp[index - 1] = '';
        setOtp(newOtp);
        inputRefs.current[index - 1]?.focus();
      }
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {!isSuccess ? (
            ...
            >
              <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-6 border border-primary/20 shadow-lg shadow-primary/10">
              ...
              {/* Segmented Inputs Container with Shake Animation */}
              <motion.div 
              ...
              <div className="mt-8 text-sm text-white/50 flex items-center justify-center gap-2">
                ...
                <button 
                  ...
                  className="text-primary hover:text-primary/80 transition-colors font-medium flex items-center gap-1 disabled:text-white/20 disabled:cursor-not-allowed"
                >
                  {resendOtp.isPending ? (
